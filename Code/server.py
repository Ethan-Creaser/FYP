...
from localise import update_position_from_dist
...
def _handle_dist(msg, loop, bcast):
    """
    DIST message: raw distances from a node's position check.
    Run trilateration on laptop and update position.
    """
    nid      = msg.get("id")
    dists    = msg.get("dists", [])
    ...
    if not dists or not anchors:
        return

    with _map_lock:
        snap = dict(node_map)

    result = update_position_from_dist(nid, dists, anchors, snap)
    if result is None:
        return
    ...
    print("[Dist] Node {} updated: ({:.3f},{:.3f},{:.3f})".format(nid, x, y, z))

    if _log:
        _log.node_position_updated(nid, x, y, z, old_x, old_y, old_z)
